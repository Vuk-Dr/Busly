...
@section('content_header')
    <div class="d-flex justify-content-between align-items-center my-4">
        <h1>Edit route : {{ $route->getName() }}</h1>
            ...
            <div class="card-body">
                ...
                <div class="row pb-4 mb-3 border-bottom">
                    <div class="col-12 col-md-8">
                        <x-adminlte-select2 name="carrier_id" label="Carrier" data-placeholder="Select carrier...">
                            ...
                            @foreach($carriers as $c)
                                <option value="{{ $c->id }}" {{ $route->carrier_id == $c->id ? 'selected' : '' }}>
                                    {{ $c->name }}
                                </option>
                            @endforeach
                        </x-adminlte-select2>
                    ...
                    <div class="col-6 col-md-2">
                        <label>Seats</label>
                        <input type="number" name="seats" placeholder="..." value="{{ $route->seats }}"
                    ...
                    <div class="col-6 col-md-2">
                        ...
                        <input type="number" name="base_price" placeholder="..." value="{{ $route->price }}"
                        ...
                        @foreach(old('stops') as $i => $stop)
                            <div class="row align-items-start station-row mb-3 pb-4 border-bottom">
                                <div class="col-md-1 pt-4 text-center">
                                    ...
                                    <input type="hidden" name="stops[{{ $i }}][order]" class="input-order"
                                           value="{{ $i+1 }}">
                                </div>
                                <div class="col-12 col-md-4">
                                    ...
                                    <select name="stops[{{ $i }}][city]"
                                        ...
                                        @foreach($cities as $c)
                                            <option
                                                value="{{ $c->id }}" {{ $stop['city'] == $c->id ? 'selected' : '' }}>
                                                {{ $c->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-2 col-8 text-center">
                                    <div class="btn-group mt-4">
                                        <button type="button" class="btn btn-outline-secondary btn-up"><i
                                                class="fas fa-arrow-up"></i></button>
                                        <button type="button" class="btn btn-outline-secondary btn-down"><i
                                                class="fas fa-arrow-down"></i></button>
                                    </div>
                                </div>
                                <div class="col-4 col-md-1  text-center mt-4">
                                    <button type="button" class="btn btn-danger btn-remove"><i class="fas fa-trash"></i>
                                    </button>
                                </div>
                            </div>
                        @endforeach
                    @else
                        @foreach($route->routeStops as $i => $stop)
                            <div class="row align-items-start station-row mb-3 pb-4 border-bottom">
                                <div class="col-md-1 pt-4 text-center">
                                    <span class="station-order badge fs-5 bg-primary">{{ $i+1 }}</span>
                                    <input type="hidden" name="stops[{{ $i }}][order]" class="input-order"
                                           value="{{ $i+1 }}">
                                    <input type="hidden" name="stops[{{ $i }}][id]" class="input-id"
                                           value="{{ $stop->id }}">
                                </div>
                                <div class="col-12 col-md-4">
                                    <label>City</label>
                                    <select name="stops[{{ $i }}][city]"
                                        ...
                                        @foreach($cities as $c)
                                            <option
                                                value="{{ $c->id }}" {{ $stop->city_id == $c->id ? 'selected' : '' }}>
                                                {{ $c->name }}
                                            </option>
                                        @endforeach
                                    </select>
                ...
                <div class="d-flex justify-content-between px-2 mt-4">
                    ...
                    @error('stops')
                    <div class="fs-6 text-danger text-bold">{{ $message }}</div> @enderror
