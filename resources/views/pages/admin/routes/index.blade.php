            ...
            <table class="table table-responsive-lg table-bordered table-striped">
                ...
                @forelse($routes as $r)
                    <tr>
                        ...
                        <td class="align-middle">{{ $r->duration() }}</td>
                        <td class="col-2 align-middle">
                            <div class="row justify-content-around">
                                <a href="{{ route('admin.routes.edit', [$r->id, 'page' => request('page')]) }}" class="btn btn-sm pt-2 px-2 px-lg-3 btn-warning ">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <x-adminlte-modal id="delete-{{ $r->id }}" theme="danger" title="Are you sure?" v-centered="true">
                                    <p class="text-center my-3">Do you really want to delete route - {{ $r->name() }}</p>
                                ...
                                <button data-toggle="modal" data-target="#delete-{{ $r->id }}" class="btn btn-danger px-2 px-lg-3"><i class="fas fa-trash"></i></button>
                            </div>
                        </td>
                    </tr>
                @empty
